...
ggplot(subset(gapminder, year == 1952),
  ...
  scale_x_log10() +
  scale_size(labels = scales::label_number()) +
  labs(
    title = "Figure 01: Life Expectancy and GDP per Capita in 1952",
    x = "GDP per capita (log10 scale, US$)",
...
ggplot(subset(gapminder, year == 2007),
  ...
  scale_size(labels = scales::label_number()) +
  labs(
    title = "Figure 02: Life Expectancy and GDP per Capita in 2007",
    x = "GDP per capita (log10 scale, US$)",
    y = "Life expectancy (years)",
    size = "Population",
    color = "Continent"
...
If not for log10 scale (`scale_x_log10()`) the x axis would stretch very far and therefore it would be dificult to see a patttern in the plot. However log10 scale (`scale_x_log10()`) compresses the x axis, and therefore msks it more pleasing to look at, and easir to tell a pattern.
...
anim2 <- ggplot(
  ...
  labs(
    ...
    x = "GDP per capita (US dollars, log10 scale)",
    y = "Life expectancy (years)",
    size = "Population",
    color = "Continent"
  ) +
 ...
 *in sync with the animation?* *(**Hint**: search labeling for `transition_states()` and `transition_time()` functions respectively)*
 A dynamic title was added to anim2 using labs(title = "Year: {frame_time}"), which updates for each frame of the animation and makes it easier to follow changes over time.
...
I improved the animation by coloring countries according to continent, expanding the axis labels, and formatting the numeric labels in a more readable way. In particular, scientific notation in the legend was removed so population values are easier to interpret.
...
```{r}
ggplot(subset(gapminder, year %in% c(2002, 2007)),
